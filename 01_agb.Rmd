...
```{r echo=F}
to.xy <- cbind(dbp$BAplot, dbp$ACD)
xl <- expression('Basal Area ' (m^2/ha))
yl <- expression("ACD " (Mg~C/ha))

plot(to.xy, pch=20, col=densCols(to.xy), xlab = xl, ylab = yl)
...
```{r echo=F}
to.xy <- cbind(dbp$WDplot2, dbp$ACD)
xl <- expression(ρ[2]~(g/cm^3))
yl <- expression("ACD " (Mg~C/ha))
...
```{r echo=F}
to.xy <- cbind(dbp$BAplot, dbp$WDplot2)
xl <- expression('Basal Area ' (m^2/ha))
yl <- expression(ρ[2]~(g/cm^3))
